...
class QSqlDriverPrivate : public QObject
{
    Q_OBJECT

public:
    QSqlDriverPrivate()
      : QObject(), isOpen(false),
        isOpenError(false),
...
{
    friend class QSQLiteEnhancedDriver;
    friend class QSQLiteResultPrivate;
public:
    explicit QSQLiteResult(const QSQLiteEnhancedDriver* db);
    ~QSQLiteResult();
...
/* ------------------------------------------------------------------------- */
class QSQLiteEnhancedDriverPrivate : public QSqlDriverPrivate
{
public:
    inline QSQLiteEnhancedDriverPrivate() : QSqlDriverPrivate(), access(0) { dbmsType = QSqlDriver::SQLite; }
    sqlite3 *access;
...
/* ------------------------------------------------------------------------- */
QSQLiteResult::QSQLiteResult(const QSQLiteEnhancedDriver* db)
    : QSqlCachedResult(db)
{
    d = new QSQLiteResultPrivate(this);
    d->access = db->impl()->access;
    const_cast<QSQLiteEnhancedDriverPrivate*>(db->impl())->results.append(this);
}
/* ========================================================================= */
...
QSQLiteResult::~QSQLiteResult()
{
    ...
    if (sqlDriver) {
        const QSQLiteEnhancedDriver * enhdrv =
                qobject_cast<const QSQLiteEnhancedDriver *>(sqlDriver);
        QSQLiteEnhancedDriverPrivate* enhpriv =
                const_cast<QSQLiteEnhancedDriverPrivate*>(enhdrv->impl());
        enhpriv->results.removeOne(this);
    }
    d->cleanup();
    delete d;
}
...
/* ------------------------------------------------------------------------- */
QSQLiteEnhancedDriver::QSQLiteEnhancedDriver(QObject * parent)
    : QSqlDriver(parent),
      d(new QSQLiteEnhancedDriverPrivate)
{
}

QSQLiteEnhancedDriver::QSQLiteEnhancedDriver (
        sqlite3 *connection, QObject *parent) :
    QSqlDriver(parent),
      d(new QSQLiteEnhancedDriverPrivate)
{
    // Q_D(QSQLiteEnhancedDriver);
    d->access = connection;
    setOpen(true);
    setOpenError(false);
}
...
bool QSQLiteEnhancedDriver::hasFeature(DriverFeature f) const
{
    switch (f) {
    case BLOB:
    case Transactions:
...
/*
   ...
   just file names.
*/
bool QSQLiteEnhancedDriver::open(const QString & db, const QString &, const QString &, const QString &, int, const QString &conOpts)
{
    //Q_D(QSQLiteEnhancedDriver);
    if (isOpen())
        close();
...
void QSQLiteEnhancedDriver::close()
{
    //Q_D(QSQLiteEnhancedDriver);
    if (isOpen()) {
...
QSqlResult *QSQLiteEnhancedDriver::createResult() const
{
    return new QSQLiteResult(this);
}

bool QSQLiteEnhancedDriver::beginTransaction()
{
...
bool QSQLiteEnhancedDriver::commitTransaction()
{
    if (!isOpen() || isOpenError())
        return false;
...
bool QSQLiteEnhancedDriver::rollbackTransaction()
{
    if (!isOpen() || isOpenError())
        return false;
...
QStringList QSQLiteEnhancedDriver::tables(QSql::TableType type) const
{
    QStringList res;
    if (!isOpen())
        return res;
...
QSqlIndex QSQLiteEnhancedDriver::primaryIndex(const QString &tblname) const
{
    if (!isOpen())
        return QSqlIndex();
...
QSqlRecord QSQLiteEnhancedDriver::record(const QString &tbl) const
{
    if (!isOpen())
        return QSqlRecord();
...
QVariant QSQLiteEnhancedDriver::handle() const
{
    //Q_D(const QSQLiteEnhancedDriver);
    return QVariant::fromValue(d->access);
}

QString QSQLiteEnhancedDriver::escapeIdentifier(const QString &identifier, IdentifierType type) const
{
    Q_UNUSED(type);
    return _q_escapeIdentifier(identifier);
}

#include "drvsqlite.moc"
