...
#ifndef ENHANCED_SQLITE_DRIVER
#define ENHANCED_SQLITE_DRIVER
...
class QSQLiteEnhancedDriverPrivate;
class QSQLiteDriver;

class REGEXLITE_EXPORT QSQLiteEnhancedDriver : public QSqlDriver
{
    Q_OBJECT

    friend class QSQLiteResult;
    QSQLiteEnhancedDriverPrivate * d;

public:
    explicit QSQLiteEnhancedDriver(QObject *parent = 0);
    explicit QSQLiteEnhancedDriver(sqlite3 *connection, QObject *parent = 0);
    ~QSQLiteEnhancedDriver();
    ...
    QString escapeIdentifier (
            const QString &identifier, IdentifierType) const Q_DECL_OVERRIDE;

protected:
    QSQLiteEnhancedDriverPrivate * impl() {
        return d;
    }
    const QSQLiteEnhancedDriverPrivate * impl() const {
        return d;
    }
};

#endif // ENHANCED_SQLITE_DRIVER
